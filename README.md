...
```
### Salida
![Captura de pantalla 2024-05-26 191650](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/assets/147757830/d7b100bc-4740-40af-b011-588fd05cba58)

## Ejercicios
[Enlace de ejrcicios](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/tree/main/Tema%205/Lagrange)

********************************************************************************************************************************************
# Newton.
## Definición.
La interpolación de Newton es un método de interpolación polinómica que utiliza diferencias divididas para construir un polinomio que pasa por un conjunto de puntos dados. La fórmula de interpolación de Newton es útil porque permite agregar puntos adicionales sin recalcular el polinomio completo.
## Algoritmo.
1. Obtener los puntos (x0, y0), (x1, y1), ..., (xn, yn).
...
function newtonInterpolation(x_vals, y_vals, x):
    n = length(x_vals)
    coef = y_vals.copy()  // Copiar los valores de y para calcular diferencias divididas
...
function main():
    ...
    // Calcular la interpolación de Newton
    resultado = newtonInterpolation(x_vals, y_vals, x)
    print("El valor interpolado en x =", x, "es", resultado)
...
```
### Salida
![Captura de pantalla (388)](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/assets/147757830/6dc72ab8-ed4e-4d9e-b35e-5dc40f0845ae)

## Ejercicios
[Enlaces de ejercicios](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/tree/main/Tema%205/Newton)
...
```
function eulerMethod(f, x0, y0, h, x_final):
    ...
    while x <= x_final:
        ...
        // Almacenar los nuevos valores de x e y
        x_vals.append(x)
        y_vals.append(y)
...
function main():
    ...
    // Definir la condición inicial y el intervalo de solución
    x0 = 0
    y0 = 1
    h = 0.1
    x_final = 1
    ...
    x_vals, y_vals = eulerMethod(f, x0, y0, h, x_final)
    
    // Imprimir los resultados
    for i in range(len(x_vals)):
        print("x =", x_vals[i], ", y =", y_vals[i])
...
```
### Salida
![Captura de pantalla (389)](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/assets/147757830/bc6b691a-086e-4d18-a920-f100f6714aaa)

## Ejercicios
[Enlace de ejercicios](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/tree/main/Tema%206/Euler)

********************************************************************************************************************************************
# Runge-Kutta.
## Definición.
El método de Runge-Kutta de cuarto orden (RK4) es un procedimiento numérico avanzado para resolver ecuaciones diferenciales ordinarias (EDOs). Es más preciso que el método de Euler y se utiliza comúnmente debido a su buena precisión y estabilidad. RK4 calcula la pendiente en varios puntos dentro del intervalo del paso para obtener una mejor aproximación de la solución.
## Algoritmo.
1. Definir la ecuación diferencial de primer orden que se desea resolver: dy/dx = f(x, y).
2. Especificar el punto inicial (x0, y0).
3. Especificar el tamaño del paso h.
...
5. Repetir el paso 4 hasta alcanzar el punto final deseado.
```
function rungeKutta4(f, x0, y0, h, x_final):
    // Inicializar las listas para almacenar los valores de x e y
    x_vals = [x0]
    y_vals = [y0]
    ...
    // Iterar hasta llegar a x_final
    while x < x_final:
        // Calcular los incrementos k1, k2, k3 y k4
        k1 = h * f(x, y)
        k2 = h * f(x + h / 2, y + k1 / 2)
        k3 = h * f(x + h / 2, y + k2 / 2)
        k4 = h * f(x + h, y + k3)
...
function main():
    ...
    // Calcular la solución usando el método de Runge-Kutta
    x_vals, y_vals = rungeKutta4(f, x0, y0, h, x_final)
...
```
### Salida
![Captura de pantalla (390)](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/assets/147757830/2adf43cf-b104-484a-8008-e5e25776fa4b)

## Ejercicios
[Enlace de ejercicios](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/tree/main/Tema%206/Runge-Kutta)

********************************************************************************************************************************************
# Taylor.
## Definición.
El método de Taylor es una técnica numérica para resolver ecuaciones diferenciales ordinarias (EDOs) utilizando la serie de Taylor. Este método expande la solución alrededor de un punto inicial utilizando derivadas sucesivas de la función. El método de Taylor de orden n proporciona una aproximación más precisa a la solución de la EDO al incluir términos adicionales de la serie de Taylor.
## Algoritmo.
1. Definir la ecuación diferencial de primer orden que se desea resolver: dy/dx = f(x, y).
2. Especificar el punto inicial (x0, y0).
...
5. Repetir el paso 4 hasta alcanzar el punto final deseado.
```
function taylorMethod(f, df_dx, df_dy, x0, y0, h, x_final):
    // Inicializar las listas para almacenar los valores de x e y
    x_vals = [x0]
    y_vals = [y0]
    ...
    // Iterar hasta llegar a x_final
    while x < x_final:
        // Calcular los términos de la serie de Taylor de segundo orden
        f_value = f(x, y)
        df_dx_value = df_dx(x, y)
        df_dy_value = df_dy(x, y)
        
        // Actualizar el valor de y usando la fórmula de Taylor de segundo orden
        y = y + h * f_value + (h^2 / 2) * (df_dx_value + f_value * df_dy_value)
...
function main():
    ...
    function df_dx(x, y):
        return 1  // Derivada de x + y respecto a x es 1
    
    // Derivada de f respecto a y
    function df_dy(x, y):
        return 1  // Derivada de x + y respecto a y es 1
    ...
    // Calcular la solución usando el método de Taylor
    x_vals, y_vals = taylorMethod(f, df_dx, df_dy, x0, y0, h, x_final)
...
```
### Salida
![Captura de pantalla (390)](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/assets/147757830/94e0fcbb-ff22-4d4f-9a94-af803347f254)

## Ejercicios
[Enlace de ejercicios](https://github.com/AlanOrgazVillegas/MetodosNumericos_T6/tree/main/Tema%206/Taylor)
********************************************************************************************************************************************
